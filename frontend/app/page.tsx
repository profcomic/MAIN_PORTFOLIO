    ...
    <div className="min-h-screen w-full max-w-full bg-transparent text-slate-100 overflow-x-hidden selection:bg-violet-400/30 selection:text-white">
        ...
        {showMenu && (
          ...
          >
            <nav
              ...
              className="flex w-full min-w-0 items-center justify-center gap-1 sm:gap-2 md:gap-4 p-1.5 sm:p-2 md:p-3 bg-slate-950/80 backdrop-blur-2xl rounded-2xl border border-cyan-300/15 shadow-[0_0_50px_rgba(34,211,238,.08)]"
            >
              ...
              <div className="grid w-full min-w-0 flex-1 grid-cols-4 gap-0.5 sm:gap-1 md:gap-2 overflow-visible space-nav">
                {navItems.map((item) => (
                  <motion.a
                    ...
                    className={`min-w-0 w-full flex items-center justify-center gap-1 sm:gap-1.5 md:gap-2 px-1 sm:px-1.5 md:px-3 lg:px-4 py-2 sm:py-2.5 rounded-xl transition-all ${
                  ...
                  >
                    ...
                    <span className="font-semibold text-[7px] min-[480px]:text-[8px] sm:text-[9px] md:text-[10px] lg:text-xs xl:text-sm whitespace-nowrap min-w-0">
                      {item.label}
                    </span>
      ...
      <main className="relative z-10 w-full max-w-full overflow-x-hidden">
        <AnimatePresence>
          {isVisible && (
            <>
              <motion.section id="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="w-full max-w-full overflow-x-hidden">
                <Hero />
              </motion.section>

              <motion.section id="about" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="min-h-screen w-full max-w-full flex items-center justify-center px-3 sm:px-6 py-16 sm:py-20 overflow-x-hidden">
                <About />
              </motion.section>

              <motion.section id="projects" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="min-h-screen w-full max-w-full px-3 sm:px-6 py-16 sm:py-20 overflow-x-hidden">
                <ProjectsSection />
              </motion.section>

              <motion.section id="contact" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="min-h-screen w-full max-w-full flex items-center justify-center px-3 sm:px-6 py-16 sm:py-20 overflow-x-hidden">
                <Contact />
              </motion.section>
            </>
          )}
